...
def query_zeros(fecha_inicio, fecha_fin, valvula):
    ...
    consulta = "SELECT angulo_sensor as Angulo, time as Fecha FROM angulos_svia WHERE angulo_sensor<= 10 and angulo_sensor>= -10 and id_sensor = '{}' AND time  >= '{}' AND time<= '{}'".valvula, format(fecha_inicio.strftime("%Y-%m-%d %H:%M:%S"), fecha_fin.strftime("%Y-%m-%d %H:%M:%S"))
...
def last_query(date):
    ...
    fecha_inicio = date - timedelta(seconds=30)
    fecha_fin = date + timedelta(seconds=30)
    cliente = InfluxDBClient(host='192.168.0.178', port=8086, username='', password='', database='SVALVIA_MCL')
    ...
    if entorno_cero.empty == False:
        ...
        for key, value in dicc_transientes.items():
            try:
                ...
                df_transiente = last_query(datetime.strptime(key.split('+')[0], '%Y-%m-%d %H:%M:%S'))
